...
const STATUS_LABELS: Record<AssignmentDto['status'], { color: string; label: string }> = {
  pending: { color: 'bg-status-pending', label: 'Queued' },
  processing: { color: 'bg-status-processing animate-pulse', label: 'Generating...' },
  completed: { color: 'bg-status-ready', label: 'Ready' },
    ...
    >
      ...
      <div className="flex h-[76px] flex-col justify-between">
        <div className="flex items-start justify-between gap-[39px] h-[25px]">
          {selectable ? (
            <h3 className={cn(
              'font-extrabold text-[18px] lg:text-[24px] leading-[140%] lg:leading-[120%] tracking-[-0.02em] text-ink line-clamp-1 min-w-0',
              'pl-7',
            )}>
              {assignment.title}
            </h3>
          ...
          <div className={cn('relative shrink-0', selectable && 'hidden')}>
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute right-0 top-8 z-20 w-[160px] bg-white rounded-2xl shadow-raised overflow-visible p-2 flex flex-col gap-1">
                  <Link
                    href={`/assignments/${assignment.id}`}
                    className="block h-8 px-2 py-1.5 text-[14px] font-medium leading-[140%] tracking-[-0.02em] text-ink rounded-lg hover:bg-surface-alt"
                  ...
                  <div className="relative">
                    <button
                      type="button"
                      className="flex items-center gap-1.5 w-full h-8 text-left px-2 py-1.5 text-[14px] font-medium leading-[140%] tracking-[-0.02em] text-ink rounded-lg hover:bg-surface-alt"
                      onClick={() => {
                    ...
                    {saveTooltip && (
                      <div className="absolute right-full top-1/2 -translate-y-1/2 mr-2 z-30 w-[180px] bg-ink text-white text-[11px] font-medium leading-[140%] rounded-xl px-3 py-2 shadow-lg pointer-events-none whitespace-normal">
                  ...
                  <button
                    className="block w-full h-8 text-left px-2 py-1.5 text-[14px] font-medium leading-[140%] tracking-[-0.02em] text-status-failed rounded-lg bg-surface-alt hover:bg-red-50"
                    onClick={() => {
                      setMenuOpen(false);
        ...
        <div className="flex h-[19px] items-center justify-between gap-4 lg:gap-6 text-[16px] lg:text-[16px] font-extrabold leading-[120%] tracking-[-0.02em] text-ink">
          <div className="min-w-0">
            <span className="font-extrabold">Assigned on </span>
            <span className="text-ink-subtle">: {formatDate(assignment.createdAt)}</span>
          </div>
          <div className="min-w-0">
            <span className="font-extrabold">Due </span>
            <span className="text-ink-subtle">: {formatDate(assignment.dueDate)}</span>
          </div>
        </div>
      </div>
    </Card>
